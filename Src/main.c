...
{
  /* USER CODE BEGIN 1 */
	while(__HAL_RCC_GET_FLAG(RCC_FLAG_SFTRST)) {
		uint32_t *f=(uint32_t *)0x8004000;
		if(!f[0] && !f[1] && !f[2] && !f[3]) {
			f=(uint32_t *)0x8008000;
		} else {
			MX_CRC_Init();
			if(HAL_CRC_Calculate(&hcrc,f,3) != f[3])
				break;
			if(HAL_CRC_Calculate(&hcrc,(uint32_t *)f[0],f[1]) != f[2])
				break;
			f=*(uint32_t **)f;
		}
		MX_IWDG_Init();
